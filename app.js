...
function renderCard(s) {
  ...
  const barsHtml = s.scores ? '<div class="score-bars">' +
  ...
  return '<div class="store-card" onclick="openDetail(\''+s.id+'\')">' +
    ...
    '<div class="metric"><div class="metric-label">女子バック目安</div><div class="metric-val">'+formatYen(backEst)+'</div><div class="metric-sub">60分x60%試算</div></div>' +
...
function renderCompareTable() {
  ...
  stores.forEach(function(s) {
    var backEst = s.price60new?Math.round(s.price60new*0.6):null;
    var cls='';
  ...
  document.getElementById('compare-table').innerHTML =
    '<thead><tr>'+['店舗名','業態','60分通常','新規最安値','指名料','特別指名','入会金','女子バック目安','口コミ総数'].map(function(c){return '<th>'+c+'</th>';}).join('')+'</tr></thead>' +
...
function renderChart() {
  ...
  var priced = stores.filter(function(s){return s.price60new;});
  new Chart(barCtx, {
    type: 'bar',
    data: {
      labels: priced.map(function(s){return s.name.length>8?s.name.slice(0,8)+'...':s.name;}),
      datasets: [
        { label:'新規最安値', data:priced.map(function(s){return s.price60new;}), backgroundColor:'#7c6af7aa', borderRadius:4 },
...
function openDetail(id) {
  ...
  var barsHtml = s.scores ?
    ...
    : '<button class="btn-primary" onclick="runAIScore(\''+s.id+'\')">AIスコアを今すぐ生成</button>';
  ...
  document.getElementById('detail-body').innerHTML =
    ...
    '<div class="detail-metric"><div class="dm-label">女子バック目安</div><div class="dm-val" style="color:var(--green)">'+formatYen(backEst)+'</div><div class="dm-sub">60分新規x60%</div></div>' +
    '</div>' + barsHtml +
